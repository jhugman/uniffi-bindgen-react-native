...
`runtimeVersion` is the version of [`uniffi-runtime-javascript` crate](https://crates.io/crates/uniffi-runtime-javascript). By default this is the exact current version of uniffi-bindgen-react-native, so currently `=0.31.0-6`.
...
This configures the build steps for the [`jsi2` flavor](jsi2.md), which loads your crate through the generic JSI player instead of generating any native code of its own.
...
jsi2:
    ...
    - x86_64
    minIosVersion: "15.1"
    bundleIdPrefix: <SAME AS the Android package name>
```

`ts` is the directory where the Typescript bindings are generated. This overrides the [`bindings`/`ts`](#bindings) directory. It is also spelled `typescript`.

`androidTargets` is the list of Android ABIs the library ships.

`minIosVersion` is the iOS deployment target stamped into the dylib and the framework's `Info.plist`.

`bundleIdPrefix` is the reverse-DNS prefix of the framework's `CFBundleIdentifier`.
