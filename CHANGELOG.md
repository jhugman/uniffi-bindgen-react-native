...
- A callback passed as a **struct field** built a fresh trampoline on every call rather than reusing the one cached for that `(callback name, JS function)` pair. Trampolines are permanently leaked by design, so each call leaked a `CallbackUserData`, a pinned `napi_ref`, a `ThreadsafeFunction` with its libuv handle, and a libffi closure, which is an unbounded leak for anything calling in a loop. [#440](https://github.com/jhugman/uniffi-bindgen-react-native/pull/440) added the cache to the argument path in `call`; this adds it to the struct-field path in `callback::marshal`, which it missed ([#469](https://github.com/jhugman/uniffi-bindgen-react-native/pull/469)).
