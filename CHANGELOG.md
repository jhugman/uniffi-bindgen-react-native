...
- `--config` accepts a 0.32 [global config file](https://mozilla.github.io/uniffi-rs/next/bindings.html#global-configuration) (`[crate-roots]`, `[defaults]`, `[crates.<name>]`) as well as the flat `uniffi.toml` override this project has always taken, and per-crate binding options are handed to the pipeline as `[bindings.react-native]`, so the documented `[bindings.typescript]` keys — custom types, renames, excludes — reach it ([#409](https://github.com/jhugman/uniffi-bindgen-react-native/pull/409)).
